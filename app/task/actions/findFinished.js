...
/**
 * @api {get} /api/task/finished Finished Task List
 * @apiPermission user
 * @apiGroup Task
 * @apiVersion 1.0.0
 * @apiDescription Lista de <code>taks</code> já foram finalizadas.
 * 
...
module.exports = (req, res, next) => {
    UserDAO.findOne({ token: req.get('Authorization') })
        .then(user => {
            TaskDAO.findAll({ user : user._id, is_finish: true, active: true })
            .then(tasks => {
